...
{
    [TestClass]
    public class Alerts_Frame_Windows_PageTC
    {
        ...
        [TestMethod]
        public async Task ClickButtonToViewPrompt()
        {
            await BasePage.Initialize();
            string Url = "https://demoqa.com/";
            await BasePage.page.GotoAsync(Url);
            Alerts_Frame_Windows_Page alerts_Frame_Windows_Page = new Alerts_Frame_Windows_Page();
            await BasePage.page.Locator("text=Alerts, Frame & Windows").ClickAsync();
            await alerts_Frame_Windows_Page.AlertWithPrompt();
            await BasePage.page.CloseAsync();
        }

        [TestMethod]
        public async Task ViewConfirmationBox()
        {
            await BasePage.Initialize();
            string Url = "https://demoqa.com/";
            await BasePage.page.GotoAsync(Url);
            Alerts_Frame_Windows_Page alerts_Frame_Windows_Page = new Alerts_Frame_Windows_Page();
            await BasePage.page.Locator("text=Alerts, Frame & Windows").ClickAsync();
            await alerts_Frame_Windows_Page.AlertWithConfirmationBox();
            await BasePage.page.CloseAsync();
        }
#endregion
